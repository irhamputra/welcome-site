...
const PAGE_TITLES = {
  [GITHUB_URL]: "irhamputra (Irham Putra) - GitHub - Microsoft Internet Explorer",
  [LINKEDIN_URL]: "Muhamad Irham Prasetyo - LinkedIn - Microsoft Internet Explorer",
};
...
export default function InternetExplorerWindow({ user }) {
  ...
  useEffect(() => {
    const title = PAGE_TITLES[LINKEDIN_URL];
    setWindowTitle("ie", title);
  }, []);
  ...
  const navigate = (url) => {
    ...
    setAddress(url);
    setInputAddress(url);
    const title = PAGE_TITLES[url] || `${url} - Microsoft Internet Explorer`;
    setWindowTitle("ie", title);
  };
  ...
  const goBack = () => {
    ...
    setAddress(url);
    setInputAddress(url);
    setWindowTitle("ie", PAGE_TITLES[url] || `${url} - Microsoft Internet Explorer`);
  };
  ...
  return (
    ...
    >
      ...
      {/* Toolbar */}
      <div style={{ display: "flex", alignItems: "center", gap: "2px", padding: "2px 4px", background: "#f1efe2", borderBottom: "1px solid #aca899", flexShrink: 0, flexWrap: "nowrap", overflowX: "auto" }}>
        <NavBtn icon="/xp/icons/Whistler Icons/Whistler - Back.png" label="Back" disabled={!canGoBack} onClick={goBack} isMobile={isMobile} />
        <NavBtn icon="/xp/icons/Whistler Icons/Whistler - Forward.png" label="Forward" disabled={!canGoForward} onClick={goForward} isMobile={isMobile} />
        <div style={{ width: 1, height: 28, background: "#aca899", margin: "0 2px", flexShrink: 0 }} />
        <NavBtn icon="/xp/icons/Windows XP Icons/IE Stop.png" label="Stop" onClick={() => {}} isMobile={isMobile} />
        <NavBtn icon="/xp/icons/Windows XP Icons/IE Refresh.png" label="Refresh" onClick={() => navigate(address)} isMobile={isMobile} />
        <NavBtn icon="/xp/icons/Windows XP Icons/IE Home.png" label="Home" onClick={() => navigate(GITHUB_URL)} isMobile={isMobile} />
        {!isMobile && (
          <>
            <div style={{ width: 1, height: 28, background: "#aca899", margin: "0 2px", flexShrink: 0 }} />
            <NavBtn icon="/xp/icons/Whistler Icons/Whistler - IE Search.png" label="Search" onClick={() => {}} isMobile={false} />
            <NavBtn icon="/xp/icons/Whistler Icons/Whistler - IE Favorites.png" label="Favorites" onClick={() => {}} isMobile={false} />
            <NavBtn icon="/xp/icons/Whistler Icons/Whistler - IE History.png" label="History" onClick={() => {}} isMobile={false} />
          </>
        )}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "4px", padding: "2px 4px", background: "#f1efe2", borderBottom: "1px solid #aca899", flexShrink: 0 }}>
        {!isMobile && <span style={{ fontSize: "11px", color: "#555", whiteSpace: "nowrap" }}>Address</span>}
        <div style={{ flex: 1, display: "flex", alignItems: "center", background: "white", border: "1px solid #7f9db9", minWidth: 0 }}>
          ...
          <input
            ...
            style={{ flex: 1, border: "none", outline: "none", fontSize: "11px", fontFamily: "Tahoma, Arial, sans-serif", padding: "2px", background: "transparent", userSelect: "text", minWidth: 0 }}
          />
        </div>
        <button onClick={handleGo} style={{ padding: isMobile ? "4px 12px" : "2px 8px", fontSize: "11px", cursor: "pointer", background: "linear-gradient(180deg,#f8f8f0,#e8e4d4)", border: "1px solid #aca899", borderRadius: "2px", flexShrink: 0 }}>Go</button>
      </div>

      {/* Content */}
      <div style={{ flex: 1, overflow: "auto", background: "white", userSelect: "text" }}>
        {isGitHub  && <GitHubPage user={user} onNavigate={navigate} linkedinUrl={LINKEDIN_URL} isMobile={isMobile} />}
        {isLinkedIn && <LinkedInPage user={user} onNavigate={navigate} githubUrl={GITHUB_URL} isMobile={isMobile} />}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1px 4px", background: "#f1efe2", borderTop: "1px solid #aca899", fontSize: "11px", color: "#555", flexShrink: 0, height: "20px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <button onClick={() => window.open(address, "_blank")} style={{ background: "none", border: "none", fontSize: "10px", color: "#0054e3", cursor: "pointer", padding: 0, textDecoration: "underline" }}>
            {isMobile ? "Open" : "Open in new window"}
          </button>
          <img src="/xp/icons/Windows XP Icons/Internet Explorer 6.png" alt="" style={{ width: 14, height: 14, opacity: 0.7 }} />
          {!isMobile && <span>Internet</span>}
        </div>
      </div>
    </div>
  );
...
/* ─── GitHub Profile Page ─────────────────────────────────────────────── */
function GitHubPage({ user, onNavigate, linkedinUrl, isMobile }) {
  ...
  return (
    <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", fontSize: "13px", background: "#f6f8fa", minHeight: "100%" }}>
      ...
      {/* GitHub top nav */}
      <div style={{ background: "#24292f", padding: isMobile ? "8px 12px" : "12px 16px", display: "flex", alignItems: "center", gap: isMobile ? "8px" : "16px" }}>
        <svg height="22" width="22" viewBox="0 0 16 16" fill="white" style={{ flexShrink: 0 }}>
          <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"/>
        </svg>
        <div style={{ flex: 1, background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 6, padding: "4px 12px", color: "rgba(255,255,255,0.6)", fontSize: "12px", minWidth: 0 }}>
          {isMobile ? "Search..." : "Search or jump to..."}
        </div>
        {!isMobile && ["Pull requests", "Issues", "Marketplace", "Explore"].map(n => (
          <span key={n} style={{ color: "white", fontSize: "12px", opacity: 0.8, cursor: "default", whiteSpace: "nowrap" }}>{n}</span>
        ))}
      </div>

      <div style={{ maxWidth: 1012, margin: "0 auto", padding: isMobile ? "12px" : "24px 16px", display: "flex", flexDirection: isMobile ? "column" : "row", gap: isMobile ? "16px" : "24px" }}>
        {/* Left sidebar / Profile header on mobile */}
        <div style={{ width: isMobile ? "100%" : 256, flexShrink: 0 }}>
          {isMobile ? (
            /* Mobile: horizontal profile header */
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12 }}>
              <img src={user.avatar_url} alt={user.login} style={{ width: 70, height: 70, borderRadius: "50%", border: "1px solid #d0d7de", flexShrink: 0 }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: "18px", fontWeight: "600", color: "#24292f", lineHeight: 1.2 }}>{user.name}</div>
                <div style={{ fontSize: "14px", color: "#57606a", fontWeight: 300, marginBottom: 4 }}>{user.login}</div>
                {user.bio && <div style={{ fontSize: "12px", color: "#24292f", lineHeight: 1.4 }}>{user.bio}</div>}
                <div style={{ fontSize: "12px", color: "#57606a", marginTop: 4 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {/* Tabs */}
          <div style={{ display: "flex", borderBottom: "1px solid #d0d7de", marginBottom: "16px", gap: "4px", overflowX: "auto", flexShrink: 0 }}>
            {tabs.map(tab => (
              <div key={tab} style={{ padding: isMobile ? "6px 10px" : "8px 16px", fontSize: "13px", cursor: "default", borderBottom: tab === "Repositories" ? "2px solid #fd8c73" : "2px solid transparent", fontWeight: tab === "Repositories" ? 600 : 400, color: "#24292f", display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap", flexShrink: 0 }}>
                {tab}
          ...
          {isMobile && (
            <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #d0d7de" }}>
              <div
                onClick={() => onNavigate(linkedinUrl)}
                style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", color: "#0969da", fontSize: "12px" }}
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="#0a66c2"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                View LinkedIn profile
              </div>
...
function LinkedInPage({ user, onNavigate, githubUrl, isMobile }) {
  return (
    <div style={{ fontFamily: "Arial, sans-serif", fontSize: "14px", background: "#f3f2ef", minHeight: "100%" }}>

      {/* LinkedIn nav */}
      <div style={{ background: "white", borderBottom: "1px solid #e0e0e0", padding: isMobile ? "0 12px" : "0 24px", display: "flex", alignItems: "center", gap: isMobile ? "8px" : "20px", height: 52, position: "sticky", top: 0, zIndex: 10 }}>
        <svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" viewBox="0 0 34 34" style={{ flexShrink: 0 }}>
        ...
        <div style={{ background: "#eef3f8", borderRadius: 4, padding: "6px 12px", display: "flex", alignItems: "center", gap: 6, width: isMobile ? 120 : 220, fontSize: "13px", color: "#666", flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }} />
        {!isMobile && ["Home", "My Network", "Jobs", "Messaging", "Notifications"].map(item => (
        ...
        {isMobile && (
          <div style={{ display: "flex", gap: 12, color: "#666", fontSize: "20px" }}>
            <span>🏠</span>
            <span>👥</span>
            <span>💼</span>
          </div>
      ...
      <div style={{ maxWidth: 860, margin: "16px auto", padding: isMobile ? "0 8px" : "0 16px", display: "flex", flexDirection: isMobile ? "column" : "row", gap: "16px" }}>
        {/* Main card */}
        <div style={{ flex: 1, minWidth: 0 }}>
          {/* Profile card */}
          <div style={{ background: "white", borderRadius: 8, border: "1px solid #e0e0e0", overflow: "hidden", marginBottom: 12 }}>
            <div style={{ height: isMobile ? 80 : 120, background: "linear-gradient(135deg, #0077b5 0%, #00a0dc 50%, #0055a5 100%)" }} />
            <div style={{ padding: isMobile ? "0 16px 16px" : "0 24px 16px", position: "relative" }}>
              <div style={{ marginTop: -44 }}>
                <img
                  src={user?.avatar_url}
                  alt="Muhamad Irham Prasetyo"
                  style={{ width: isMobile ? 72 : 88, height: isMobile ? 72 : 88, borderRadius: "50%", border: "3px solid white", display: "block", background: "white" }}
                />
              </div>
              {!isMobile && (
              ...
              <div style={{ marginTop: 8 }}>
                <div style={{ fontSize: isMobile ? "18px" : "20px", fontWeight: 600, color: "#191919" }}>Muhamad Irham Prasetyo</div>
                ...
                <div style={{ fontSize: "13px", color: "#0a66c2", marginBottom: isMobile ? 8 : 0 }}>500+ connections</div>
                {isMobile && (
                  <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                    <button style={{ flex: 1, background: "white", color: "#0a66c2", border: "1px solid #0a66c2", borderRadius: 16, padding: "6px 12px", fontSize: "13px", fontWeight: 600, cursor: "pointer" }}>Message</button>
                    <button style={{ flex: 1, background: "#0a66c2", color: "white", border: "none", borderRadius: 16, padding: "6px 12px", fontSize: "13px", fontWeight: 600, cursor: "pointer" }}>Connect</button>
                  </div>
                )}
                {!isMobile && (
          ...
          {/* About */}
          <div style={{ background: "white", borderRadius: 8, border: "1px solid #e0e0e0", padding: isMobile ? "12px 16px" : "16px 24px", marginBottom: 12 }}>
          ...
          {/* Also on — mobile: show inline */}
          {isMobile && (
            <div style={{ background: "white", borderRadius: 8, border: "1px solid #e0e0e0", padding: "12px 16px", marginBottom: 12 }}>
...
function ErrorPage({ address }) {
  return (
    <div style={{ padding: "40px 16px", fontFamily: "Tahoma, sans-serif" }}>
      <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
        <img src="/xp/icons/Windows XP Icons/Internet Explorer 6.png" alt="" style={{ width: 32, height: 32, opacity: 0.6, marginTop: 4, flexShrink: 0 }} />
...
/* ─── NavBtn ──────────────────────────────────────────────────────────── */
function NavBtn({ icon, label, disabled, onClick, isMobile }) {
  return (
    <button
      title={label}
      onClick={onClick}
      disabled={disabled}
      style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1px", padding: isMobile ? "2px 6px" : "2px 4px", background: "transparent", border: "1px solid transparent", borderRadius: "2px", cursor: disabled ? "default" : "pointer", opacity: disabled ? 0.4 : 1, minWidth: isMobile ? "28px" : "32px", fontSize: "10px", color: "#333", flexShrink: 0 }}
